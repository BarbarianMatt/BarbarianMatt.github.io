	...
	root.expandFleet = function (input, battleSide) {
		...
		for (var unitType in UnitType) {
			...
			for (var i = 0; i < counter.count; i++) {
				...
				if (unit.sustainDamageHits > 0 &&
					...
					//console.log(counter.participants);
		...
		function nomadPromissary(fleet,level){
			...
			for (var i = 0; i < perfer.length; i++){
				if (fleet.some(unitIs(perfer[i])))
					unitType = perfer[i];
			}
			if (unitType !== null){
				for (var i = 0; i < fleet.length; i++) {
					if (fleet[i].type === unitType && !fleet[i].isDamageGhost)
						unit = fleet[i];
				}
			}
		...
		function filterFleet() {
			...
			var result = this.filter(function (unit) {
				...
				if (battleType === root.BattleType.Space)
					return unit.typeShip || (unit.type === UnitType.Mech && thisSideOptions.race === root.Race.Nomad && unit.isDamageGhost && !thisSideOptions.articlesOfWar) && (allowed[unit.type]>=0 || unit.isDamageGhost);
