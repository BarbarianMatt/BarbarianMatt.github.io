(function (root) {
	...
	root.calculator = (function () {
		...
		function solveProblem(problem, battleType, attackerFull, defenderFull, options, input ) {
			...
			if (attackerBoost !== undefined || defenderBoost !== undefined || // boosts apply to the first round only
				...
				if (magenDefenseActivated)
					...
					attackerTransitionsFactory = function () {
						return computeFleetTransitions(problem.attacker, game.ThrowType.Battle, attackerBoost, attackerRollBoost, attackerReroll);
					};
				var defenderTransitionsFactory = function () {
					return computeFleetTransitions(problem.defender, game.ThrowType.Battle, defenderBoost, defenderRollBoost, defenderReroll);
				};
			...
			if (magenDefenseActivated && (attackerBoost !== undefined || attackerReroll || attackerRollBoost !== undefined)) {
				...
				var defenderTransitionsFactory = function () {
					return computeFleetTransitions(problem.defender, game.ThrowType.Battle, boost(battleType, options.defender, options.attacker, problem.defender, false), rollBoost(battleType, options.defender, options.attacker, problem.defender, false,attackerFull));
				};
				applyTransitions(problem, attackerTransitionsFactory, defenderTransitionsFactory, options, effectsFlags);
		...
		function propagateProbabilityUpLeft(problem, battleType, attackerFull, defenderFull, options,input ) {
			...
			var attackerTransitions = computeFleetTransitions(problem.attacker, game.ThrowType.Battle, boost(battleType, options.attacker, options.defender, problem.attacker, false), rollBoost(battleType, options.attacker, options.defender, problem.attacker, false,defenderFull));
			var defenderTransitions = computeFleetTransitions(problem.defender, game.ThrowType.Battle, boost(battleType, options.defender, options.attacker, problem.defender, false), rollBoost(battleType, options.defender, options.attacker, problem.defender, false,attackerFull));
			
			if (options.attacker.race === game.Race.L1Z1X && battleType === game.BattleType.Ground) {
				var harrowTransitions = bombardmentTransitionsVector(attackerFull, defenderFull, options,input);
				...
				for (var d = distr.columns - 1; 0 < d; d--) {
					...
					if (winnuFlagshipRelevant) {
						if (options.attacker.race === game.Race.Winnu && modifyWinnuFlagship(problem.attacker, problem.defender, d)) {
							attackerTransitions = computeFleetTransitions(problem.attacker, game.ThrowType.Battle, boost(battleType, options.attacker, options.defender, problem.attacker, false), rollBoost(battleType, options.attacker, options.defender, problem.attacker, false,defenderFull));
						}
						if (options.defender.race === game.Race.Winnu && modifyWinnuFlagship(problem.defender, problem.attacker, a)) {
							defenderTransitions = computeFleetTransitions(problem.defender, game.ThrowType.Battle, boost(battleType, options.defender, options.attacker, problem.defender, false), rollBoost(battleType, options.defender, options.attacker, problem.defender, false,attackerFull));
						}
					}
		...
		function computeFleetTransitions(fleet, throwType, modifier, modifierRoll, reroll) {
			modifier = modifier || 0;
			modifierRoll = modifierRoll || 0;
			var result = [[1]];
			for (var a = 1; a <= fleet.length; ++a) {
				var unit = fleet[a - 1];
				var thisUnitTransitions = computeUnitTransitions(unit, throwType, modifier, modifierRoll, reroll);
		...
		function computeSelectedUnitsTransitions(fleet, throwType, predicate, modifier, extraRoll, reroll) {
			var result = [[1]];
			var currentTransitions = [[1]];
			for (var i = 0; i < fleet.length; i++) {
				var unit = fleet[i];
				if (predicate(unit)) {
					var transitions = computeUnitTransitions(unit, throwType, modifier,0, reroll);
			...
			if (bestUnit) {
				...
				var transitions = computeUnitTransitions(dummyUnit, throwType, modifier,extraRoll, reroll);
				currentTransitions = slideMultiply(currentTransitions, transitions)
				result.push(currentTransitions);
			}
			return result;
			...
			return [
					...
					execute: function (problemArray, attackerFull, defenderFull, options) {
						...
						function getSpaceCannonTransitionsVector(fleetFull, opponentFleetFull, thisSideOptions, opponentSideOptions) {
							...
							if (thisSideOptions.plasmaScoring)
								additional+=1;
							if (thisSideOptions.argentCommander)
								additional+=1;
							if (getKeyByValue(options,thisSideOptions)==='attacker' && thisSideOptions.argentStrikeWingSpaceCannonA && thisSideOptions.race !== game.Race.Argent)
								additional+=1;
							if (getKeyByValue(options,thisSideOptions)==='defender' && thisSideOptions.argentStrikeWingSpaceCannonD && thisSideOptions.race !== game.Race.Argent)
								additional+=1;
							if (thisSideOptions.jolnarCommander)
								reroll = true;
							vector = fleetTransitionsVectorWithAdditionalRoll(spaceCannonFleet, game.ThrowType.SpaceCannon, modifier, additional, reroll);
				...
				},
				{
					...
					execute: function (problemArray, attackerFull, defenderFull, options,input ) {
						problemArray.forEach(function (problem) {
							var attackerTransitionsVector = bombardmentTransitionsVector(attackerFull, defenderFull, options,input);
							var attackerTransitions = scale(attackerTransitionsVector, problem.attacker.length + 1);
				...
				},
				{
					...
					execute: function (problemArray, attackerFull, defenderFull, options) {
						problemArray.forEach(function (problem) {
							...
							var pdsDefender = defenderFull.filter(groundForceSpaceCannon);
							...
							if (options.defender.argentStrikeWingSpaceCannonD && options.defender.race !== game.Race.Argent)
								additional+=1;
							defenderTransitionsVector = fleetTransitionsVectorWithAdditionalRoll(pdsDefender, game.ThrowType.SpaceCannon, defenderModifier, additional, reroll);
		...
		function rollBoost(battleType, sideOptions, opponentOptions, fleet, firstRound,fleetFull) {
			var result = undefined;
			for (var i = 0; i < rollBoosts.length; i++) {
				if (!firstRound && rollBoosts[i].firstRoundOnly) continue;

				var boost = rollBoosts[i].apply(battleType, sideOptions, opponentOptions, fleet,fleetFull);
				if (boost && !result) {
					result = boost;
					continue;
				}
		...
		function bombardmentTransitionsVector(attackerFull, defenderFull, options,input ) {
			...
			if (options.attacker.argentStrikeWingBombardmentA && options.attacker.race !== game.Race.Argent)
				additional+=1;
			var resultTransitionsVector=fleetTransitionsVectorWithAdditionalRoll(bombardmentAttacker, game.ThrowType.Bombardment, attackerModifier, additional,reroll);
		...
		function fleetTransitionsVectorWithAdditionalRoll(fleet, throwType, modifier, roll, reroll) {
			var fleetInflicted = computeFleetTransitions(fleet, throwType, modifier, 0, reroll).pop();
			var bestUnit = getUnitWithLowest(fleet, game.ThrowValues[throwType]);
			if (bestUnit) {
