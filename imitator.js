(function (root) {
	...
	root.tgs = { attacker: {tg:0, yinAgentUses:0, maneuveringJetsUses:0, directHitUses:0}, defender: {tg:0, yinAgentUses:0, maneuveringJetsUses:0, directHitUses:0} };
	root.imitator = (function () {
		...
		function imitateBattle(attackerFull, defenderFull, battleType, options,input) {
			...
			for (var i = 0; i < actions.length; i++) {
				...
				if (action.appliesTo === battleType)
					action.execute(attacker, defender, attackerFull, defenderFull, options,input);
				if (i === 0) {
			...
			while (hasUnits(attacker) && hasUnits(defender) || (doAtLeastOneRound && round === 0)) {
				...
				if (options.attacker.race === game.Race.L1Z1X && attacker.some(unitIs(game.UnitType.Flagship))) {
					attackerInflictedToNonFighters = rollDice(attacker.filter(flagshipOrDreadnought), game.ThrowType.Battle, attackerBoost, attackerReroll, attackerBoostRoll);
					attackerInflictedToEverything = rollDice(attacker.filter(not(flagshipOrDreadnought)), game.ThrowType.Battle, attackerBoost, attackerReroll, attackerBoostRoll);
				} else
					attackerInflictedToEverything = rollDice(attacker, game.ThrowType.Battle, attackerBoost, attackerReroll, attackerBoostRoll);
				if (options.defender.race === game.Race.L1Z1X && defender.some(unitIs(game.UnitType.Flagship))) {
					defenderInflictedToNonFighters = rollDice(defender.filter(flagshipOrDreadnought), game.ThrowType.Battle, defenderBoost, defenderReroll, defenderBoostRoll);
					defenderInflictedToEverything = rollDice(defender.filter(not(flagshipOrDreadnought)), game.ThrowType.Battle, defenderBoost, defenderReroll, defenderBoostRoll);
				} else
					defenderInflictedToEverything = rollDice(defender, game.ThrowType.Battle, defenderBoost, defenderReroll, defenderBoostRoll);
				...
				if (options.attacker.race === game.Race.L1Z1X && battleType === game.BattleType.Ground) { // Harrow
					...
					}).execute(attacker, defender, attackerFull, defenderFull, options,input);
				}
				...
				/*if (round>1000){
					root.imitationIterations = root.imitationIterations/10;
					attacker.splice(0);
					break;
				}*/
			}
		...
		function rollDice(fleet, throwType, modifier, reroll, extraModifier) {
			modifier = modifier || 0;
		...
		function initPrebattleActions() {
			return [
				{
					...
					execute: function (attacker, defender, attackerFull, defenderFull, options, input) {
						...
						var attackerInflicted = rollDice(attackerFull.filter(hasSpaceCannon), game.ThrowType.SpaceCannon, attackerModifier, attackerReroll);
						if (options.attacker.plasmaScoring) 
							attackerInflicted += fromAdditionalRoll(attackerFull, game.ThrowType.SpaceCannon, attackerModifier,attackerReroll);
						...
						var defenderInflicted = rollDice(defenderFull.filter(hasSpaceCannon), game.ThrowType.SpaceCannon, defenderModifier, defenderReroll);
						if (options.defender.plasmaScoring) 
							defenderInflicted += fromAdditionalRoll(defenderFull, game.ThrowType.SpaceCannon, defenderModifier, defenderReroll);
						...
						var defenderList = applyDamage(defender, attackerInflicted, options.defender, notGroundForce, gravitonLaserUnitHittable(options.attacker));
						var attackerList = applyDamage(attacker, defenderInflicted, options.attacker, notGroundForce, gravitonLaserUnitHittable(options.defender));
						aDeadUnits=[];
						dDeadUnits=[];
				...
				},
				{
					...
					execute: function (attacker, defender, attackerFull, defenderFull, options, input) {
						...
						var attackerReroll= options.attacker.jolnarCommander ? true: false;
						var defenderReroll= options.defender.jolnarCommander ? true: false;
						var attackerInflicted = rollDice(attackerBarrageUnits, game.ThrowType.Barrage,0, attackerReroll);
						var defenderInflicted = rollDice(defenderBarrageUnits, game.ThrowType.Barrage,0, defenderReroll);

						if (options.attacker.argentCommander) 
							attackerInflicted += fromAdditionalRoll(attackerBarrageUnits, game.ThrowType.Barrage,0, attackerReroll);
						if (options.defender.argentCommander) 
				...
				},
				{
					...
					execute: function (attacker, defender, attackerFull, defenderFull, options,input) {
						var bombardmentPossible = !options.defender.conventionsOfWar && (
							!defenderFull.some(unitShield()) // either there are no defending PDS or Arborec Mechs
						...
						var attackerInflicted = rollDice(bombardmentAttacker, game.ThrowType.Bombardment, attackerModifier,reroll);
						if (options.attacker.plasmaScoring) 
							attackerInflicted += fromAdditionalRoll(bombardmentAttacker, game.ThrowType.Bombardment, attackerModifier, reroll);
						if (options.attacker.argentCommander) 
							attackerInflicted += fromAdditionalRoll(bombardmentAttacker, game.ThrowType.Bombardment, attackerModifier, reroll);
						if (options.attacker.argentStrikeWingBombardmentA && options.attacekr.race !== game.Race.Argent) 
							attackerInflicted += fromAdditionalRoll(bombardmentAttacker, game.ThrowType.Bombardment, attackerModifier, reroll);
				...
				},
				{
					...
					execute: function (attacker, defender, attackerFull, defenderFull, options, input) {
						...
						var defenderInflicted = rollDice(spaceCannonUnits, game.ThrowType.SpaceCannon, defenderModifier, reroll);

						if (options.defender.plasmaScoring) 
							defenderInflicted += fromAdditionalRoll(spaceCannonUnits, game.ThrowType.SpaceCannon, defenderModifier, reroll);
						if (options.defender.argentCommander) 
							defenderInflicted += fromAdditionalRoll(spaceCannonUnits, game.ThrowType.SpaceCannon, defenderModifier, reroll);
						if (options.defender.argentStrikeWingSpaceCannonD && options.defender.race !== game.Race.Argent) 
							defenderInflicted += fromAdditionalRoll(spaceCannonUnits, game.ThrowType.SpaceCannon, defenderModifier, reroll);
			...
			function fromAdditionalRoll(fleet, throwType, modifier, reroll) {
				...
				if (bestUnit) {
					...
					return rollDice([unitWithOneDie], throwType, modifier,reroll);
				}
				return 0;
			}
		}
		...
		function unitShield() {
			return function (unit) {
				return unit.planetaryShield && !unit.isDamageGhost;
			};
		}
		...
		function addUnitBasedOnRace(deadUnitType,battleSide,opponentSide,option,fleet,opponentFleet,input){
			var thisSideCounters = input[root.SideUnits[battleSide]];
			var counter = thisSideCounters[deadUnitType] || { count: 0 };
			var standardUnits = root.MergedUnits[option[battleSide].race];
			var upgradedUnits = root.MergedUpgrades[option[battleSide].race];
			var unit = (counter.upgraded ? upgradedUnits : standardUnits)[deadUnitType];
			var opponentMentakFlagship = input.battleType === root.BattleType.Space && option[opponentSide].race === root.Race.Mentak && opponentFleet.some(unitIs(game.UnitType.Flagship));
			fleet.push(unit);
			if (unit.sustainDamageHits > 0 &&
		...
		function organizeFleet(fleet,battleSide,option,input){
			var virusFlagship = input.battleType === root.BattleType.Space && option[battleSide].race === root.Race.Virus && fleet.some(unitIs(game.UnitType.Flagship));
